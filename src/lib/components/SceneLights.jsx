...
function StaticLights({ lighting }) {
  ...
  return (
    <>
      {ambient && <ambientLight color={ambient.color} intensity={ambient.intensity ?? 1} />}
      {spot && (
        <spotLight ref={spotRef}
          position={spot.position} angle={spot.angle ?? Math.PI / 6}
          penumbra={spot.penumbra ?? 0.5} intensity={spot.intensity ?? 1}
          color={spot.color} castShadow
          shadow-mapSize-width={2048} shadow-mapSize-height={2048} shadow-bias={-0.001}
        />
      )}
      {directional && (
        <directionalLight ref={dirRef}
          position={directional.position} intensity={directional.intensity ?? 1}
          color={directional.color} castShadow
          shadow-mapSize-width={2048} shadow-mapSize-height={2048}
        />
      )}
      {point && (
        <pointLight
          position={point.position} intensity={point.intensity ?? 1} color={point.color}
        />
      )}
    </>
  )
...
function AnimatedLights({ initial, final, driver }) {
  ...
  const initAmbientColor  = useMemo(() => new THREE.Color(initial.ambient?.color  ?? '#000'), [initial.ambient?.color])
  const finalAmbientColor = useMemo(() => new THREE.Color(final.ambient?.color    ?? '#000'), [final.ambient?.color])
  ...
  useFrame(() => {
    ...
    if (ambientRef.current) {
      ambientRef.current.color.lerpColors(initAmbientColor, finalAmbientColor, tv)
      ambientRef.current.intensity = initAmbIntensity + (finalAmbIntensity - initAmbIntensity) * tv
    }
    if (spotRef.current && spot) {
  ...
  return (
    <>
      {(initial.ambient || final.ambient) && (
        <ambientLight ref={ambientRef} color={initAmbientColor} intensity={initAmbIntensity} />
      )}
      {spot && (
        <spotLight ref={spotRef}
          position={spot.position} angle={spot.angle ?? Math.PI / 6}
      ...
      {dir && (
        <directionalLight ref={dirRef}
          position={dir.position} intensity={0}
          color={dir.color} castShadow
          shadow-mapSize-width={2048} shadow-mapSize-height={2048}
        />
      )}
    </>
